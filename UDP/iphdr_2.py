...
class Iphdr:
    def __init__(self, tot_len, protocol, saddr, daddr):
        ...
        self.check = 0
        self.saddr = inet_aton(saddr)
        self.daddr = inet_aton(daddr)
    
    def pack_Iphdr(self):
        packed = b''
        packed += struct.pack('!BBH', self.ver_len, self.tos, self.tos_len)
...
def getIP(unpakced_ipheader):
    src_ip = inet_ntoa(unpacked_iphdr[8])
    dst_ip = inet_ntoa(unpacked_iphdr[9])
...
print('Packet size:{} Protocol:{} IP:{}'.format(getPacketSize(unpacked_iphdr), getPacketSize(unpacked_iphdr), getIP(unpacked_iphdr)))
